...
for host in ${ip_list[@]}; do
  echo $host
  # ssh -f -n $host "bash -c 'cd /home/jd/blake/kimi/megatron-lm-musa-patch/examples/kimi-k2/; NNODES=$NNODES NRANK=$COUNT MASTER_ADDR=$MASTER_ADDR MASTER_PORT=$MAIN_PROCESS_PORT OUTPUT_DIR=$OUTPUT_DIR bash kimi-k2/pretrain_kimi_1000B_platform.sh > $LOG_DIR/log.$COUNT.$host 2>&1 &'"
  # 1 host
  # ssh -f -n $host "bash -c 'cd /home/jd/blake/kimi/megatron-lm-musa-patch/examples/kimi-k2; NNODES=1 NRANK=0 OUTPUT_DIR=$OUTPUT_DIR bash run_kimi_1host.sh > $LOG_DIR/log.$COUNT.$host 2>&1 &'"
  # node check h2d d2h
  ssh -f -n $host "bash -c 'cd /home/jd/blake/kimi/d2h_h2d_test; NNODES=1 NRANK=0 OUTPUT_DIR=$OUTPUT_DIR bash run_test.sh > $LOG_DIR/log.$COUNT.$host 2>&1 &'"
  # node check fa test
  # ssh -f -n $host "bash -c 'cd /home/jd/blake/kimi/fa_test; NNODES=1 NRANK=0 OUTPUT_DIR=$OUTPUT_DIR bash run_mudnn_bench_fa.sh 8 100000 > $LOG_DIR/log.$COUNT.$host 2>&1 &'"

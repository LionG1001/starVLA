...
run_mpi_test() {
        ...
        local timeout_sec="${MPI_TIMEOUT:-360}"
        ...
        /home/jd/blake/kimi/cluster/run_with_ulimit.sh /usr/local/musa/mccl_test/alltoall_perf -n 3 -b 128M -e 20G -f 2 -g 1 -t 1 > "$output_file" 2>&1
        
        # /home/jd/blake/kimi/cluster/run_with_ulimit.sh /usr/local/musa/mccl_test/all_reduce_perf -n 3 -b 128M -e 20G -f 2 -g 1 -t 1 > "$output_file" 2>&1
        # /home/jd/blake/kimi/cluster/run_with_ulimit.sh /usr/local/musa/mccl_test/all_gather_perf -n 3 -b 128M -e 20G -f 2 -g 1 -t 1 > "$output_file" 2>&1
        # /usr/local/musa/mccl_test/all_reduce_perf -n 3 -b 512M -e 20480M -f 2 -g 1 -t 1 > "$output_file" 2>&1
        # /home/jd/blake/kimi/cluster/run_with_ulimit.sh /usr/local/musa/mccl_test/all_reduce_perf -n 3 -b 512K -e 20M -f 2 -g 1 -t 1 > "$output_file" 2>&1
        # /usr/local/musa/mccl_test/alltoall_perf -n 3 -b 512M -e 20480M -f 2 -g 1 -t 1 > "$output_file" 2>&1
        local exit_code=$?
...
if [[ $NETCHECK -eq 1 ]]; then
    ...
    for gid in "${!phase1_ips[@]}"; do
        logfile="${LOG_DIR}/mccl_allreduce_groupp1_${gid}.txt"
        ips=(${phase1_ips[$gid]})
    ...
    for ((gid=1; gid<=group_id; gid++)); do
        ...
        logfile="${LOG_DIR}/mccl_allreduce_groupp2_${gid}.txt"

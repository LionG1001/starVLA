...
class VLATrainer(TrainerUtils):
    def __init__(self, cfg, model, vla_train_dataloader, optimizer, lr_scheduler, accelerator):
        ...
        self.gpu_peak_tflops = getattr(cfg.trainer, "gpu_peak_tflops", 500.0)
    ...
    def _log_metrics(self, metrics):
        ...
        if self.completed_steps % self.config.trainer.logging_frequency == 0 and dist.get_rank() == 0:
            ...
            log_msg = f"Step {self.completed_steps}, Loss: {metrics}{mfu_str}"
    ...
    def _estimate_model_tflops(self, batch_size, seq_len=2048):
        ...
        """
        # Get model config
        hidden_size = getattr(self.model.qwen_vl_interface.model.config, 'hidden_size', 2048)
        num_layers = getattr(self.model.qwen_vl_interface.model.config, 'num_hidden_layers', 36)
        intermediate_size = getattr(self.model.qwen_vl_interface.model.config, 'intermediate_size', 5504)
        ...
        total_flops = transformer_flops + vision_flops + action_flops

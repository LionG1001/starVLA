...
class Qwenvl_OFT(baseframework):
    ...
    def _gather_action_token_embeddings(
        ...
        # topk 需要浮点数类型，将 masked_pos 转换为 float
        masked_pos_float = masked_pos.float()
        topk_pos = masked_pos_float.topk(k=self.chunk_len, dim=-1).values     # [B, chunk_len] 未排序
        # 转回 long 类型用于索引
        topk_pos = topk_pos.long()
        # 时间顺序排序
